...
LCChat 是基于 **Go 1.25** 的即时通信**后端**仓库，采用微服务架构：

- **Gateway** 暴露 HTTP API
- 服务间通过 **gRPC + Protobuf**（含 validate 规则）同步协作
- 跨服务副作用与消息下行通过 **Kafka + Outbox + Debezium CDC** 最终一致投递
- 存储使用 **MySQL（GORM）**、**Redis Stack（含 RedisBloom）**、**MinIO**
- 依赖注入使用 **Google Wire**

> **本仓库只包含后端服务与基础设施编排。**  
> 桌面客户端（Electron + Vue）在**独立仓库**维护；本地若存在被忽略的 `web/` 目录，仅为联调时的旁路检出，**不随本仓库版本管理**（见根目录 `.gitignore`）。
...
| **msg** | `apps/msg` | 消息落库、seq、会话、撤回、已读、推送事件生产、群成员会话投影 | gRPC `:9092`，metrics `:9192` |
...
| relation | `user_relations`、好友申请 | 不查/改账号表与用户资料表 |
| group | `groups`、`group_members`、`group_join_requests` | 不维护消息表、不管理 WebSocket |
| msg | `message`、`conversation`、`group_conversation` | 不维护在线连接、不直接改群成员事实 |
...
- **消息**：单聊/群聊发送、单会话拉取、多会话位点批量同步、撤回、会话列表、已读、免打扰与置顶
- **实时**：WebSocket 下行、多端同步、设备在线状态、消息 ACK 位点、关系/群申请等实时提醒
...
| 编排 | Docker Compose（单机 / 多副本 scale） |

客户端（Electron + Vue）不在本仓库内，对接时只需配置 Gateway CORS 与 Connect Origin 白名单（见下文「关键配置」）。
...
1. **发送**：gateway → msg `SendMessage` → 校验 relation/group → 幂等（Redis + DB）→ Redis 分配 seq → 同事务写 `message` + outbox(`msg.push`) → 更新会话派生数据  
2. **下行**：Debezium → Kafka `msg.push` → message-push（P2P 查路由 / 群聊查成员后扩散）→ 按 Redis `user:routing:{uuid}` 分组 → 调对应 connect 节点 `PushToUser` / `PushToDevice` → WebSocket 写出  
3. **非阻断**：`message` 与 `msg.push` Outbox 同事务提交成功即视为发送成功；会话派生更新 / Kafka / 实时推送失败只告警，客户端靠 `PullMessages` / `BatchSyncMessages` 自愈  
...
| `profile_display_changed` | user（直接 Producer） | auth | 回写登录展示昵称/头像 |
| `account.deleted` | auth Outbox | user / relation / group 等 | 注销后异步清理 |
| `group.cache` | group Outbox | **group** 投影 Redis；**msg** 投影会话成员资格 | 群写模型 → 缓存/会话投影（独立消费组） |
| `msg.push` | msg Outbox | message-push | 消息/撤回/已读等下行 |
| `realtime.push` | relation / group（直接 Producer） | message-push | 好友、群申请等实时提醒 |
| `*.redis.invalidate` | auth / user / relation | 各服务 Redis 补偿消费者 | 缓存写失败后的安全 `DEL` 补偿 |
...
```text
LCChat/
├── apps/                      # 8 个微服务（cmd + internal + pb）
│   ├── gateway/
│   ├── auth/
│   ├── user/
│   ├── relation/
│   ├── group/
│   ├── msg/                   # DDD：domain + usecase
│   ├── connect/
│   └── message-push/
├── proto/                     # Protobuf 源文件（契约源头）
├── third_party/validate/      # buf import 依赖（validate.proto，须入库）
...
│   └── nginx/                 # 多副本 LB 配置（gateway / connect）
├── scripts/                   # CDC 注册、迁移、校验脚本
├── tests/e2e/                 # Docker Compose 端到端测试（e2e build tag）
├── doc/                       # 项目文档（唯一正式文档入口）
├── docker-compose.yml         # 基础设施 + 8 服务
├── docker-compose.scale.yml   # 多副本扩展示例
├── Dockerfile                 # 多阶段：buf generate + 编译全部服务二进制
...
| 产物 | 是否入库 | 说明 |
| --- | --- | --- |
| `*.pb.go` / `*.pb.validate.go` | **否** | 由 `make proto` 或 Docker builder 阶段 `buf generate` 产出 |
| `apps/*/cmd/wire_gen.go` | **是** | Wire 生成，**不要手改**；改装配改 `wire.go` / `providers.go` |
| `third_party/validate/validate.proto` | **是** | buf import 依赖，必须保留 |

克隆后或修改 `proto/` 后，本地编译前必须先执行 `make tools && make proto`。
...
| 变量 | 说明 |
| --- | --- |
| `JWT_SECRET` | auth / gateway / connect **必须相同**；生产务必替换为足够随机的长串 |
| `EMAIL_*` / `EMAIL_AUTH_CODE` | 验证码邮件（如 QQ SMTP） |
| `MINIO_*` | 对象存储凭据 |
| `CONNECT_ALLOWED_ORIGINS` | WebSocket Origin 白名单；浏览器/桌面端跨域连接必须配置 |
| `CORS_ALLOWED_ORIGINS` | Gateway CORS 允许来源（不支持 `"*"` 带凭据） |
...
会启动：MySQL、Redis Stack、Kafka、`kafka-topics-init`（业务 Topic 固定 3 分区）、Kafka Connect、`cdc-init`（注册 Outbox Connector）、MinIO，以及 8 个业务服务（镜像 builder 阶段预编译二进制）。
...
仅启动基础设施（本地 `go run` 各服务时）：

```bash
docker compose up -d mysql redis kafka kafka-connect minio
docker compose up --force-recreate kafka-topics-init cdc-init
```

多副本扩展可参考 `docker-compose.scale.yml` 与 [`doc/guides/docker-compose三副本验证.md`](doc/guides/docker-compose三副本验证.md)。
...
# 配置好环境变量后（见 deploy/env/chatserver.env.example）
...
`apps/*/cmd/wire_gen.go` 入库，由 Wire 生成，**不要手改**。改依赖装配时改 `wire.go` / `providers.go` 后重新生成对应 `wire_gen.go`。

### 与独立客户端联调

客户端在独立仓库。联调时请保证：

1. Gateway 可达（默认 `http://127.0.0.1:8080`）
2. Connect WebSocket 可达（默认 `ws://127.0.0.1:8081`）
3. `CORS_ALLOWED_ORIGINS` / `CONNECT_ALLOWED_ORIGINS` 包含客户端 Origin
4. `JWT_SECRET` 与后端一致（由 auth 签发，gateway/connect 校验）
...
# 完整 Docker Compose 端到端测试（会创建真实数据并可能重启依赖）
LCCHAT_E2E=1 go test -tags=e2e -count=1 -v ./tests/e2e
```

覆盖范围、环境变量与按模块运行方式见 [`doc/ops/端到端功能测试.md`](doc/ops/端到端功能测试.md)。
...
每个会调用 `util.GenIDString()` 的进程必须配置**唯一** `SNOWFLAKE_NODE_ID`（0–1023）。Compose 默认：auth=101，user=201，group=301，msg=401。多副本**不能**共享同一节点号，否则可能产生重复 ID。
...
| 本地运行、配置、监控、E2E | [`doc/ops/`](doc/ops/) |
| 多副本验证 | [`doc/guides/docker-compose三副本验证.md`](doc/guides/docker-compose三副本验证.md) |
...
- **错误**：业务错误统一 `pkg/apperr`；跨 gRPC 用 `ToStatus` / `FromStatus`（出站前 `Sanitize`），禁止用裸 `status.Error` 表达业务码
- **日志**：中文消息 + 英文 field key；最终请求日志只在边界拦截点输出；禁止记录 password / 验证码 / 完整 token
- **gRPC 重试**：默认不重试；仅对显式 full method 白名单配置 ServiceConfig retryPolicy（只读/幂等方法），写接口默认禁止
