...
// PermissionChecker 通过 user-service 校验消息发送权限。
//
// msg-service 只持久化已授权的消息；好友、黑名单、群成员等跨领域规则
// 通过 gRPC 查询 user-service，避免 message 领域直接依赖用户/社交表。
...
func NewPermissionChecker(conn *grpc.ClientConn) *PermissionChecker {
	...
	return &PermissionChecker{
		friendClient:    userpb.NewFriendServiceClient(conn),
		blacklistClient: userpb.NewBlacklistServiceClient(conn),
		groupClient:     userpb.NewGroupServiceClient(conn),
	}
}
...
func (c *PermissionChecker) checkP2P(ctx context.Context, fromUUID, targetUUID string) error {
	...
	blockedBySelf, err := c.blacklistClient.CheckIsBlacklist(ctx, &userpb.CheckIsBlacklistRequest{
		UserUuid:   fromUUID,
	...
	blockedByPeer, err := c.blacklistClient.CheckIsBlacklist(ctx, &userpb.CheckIsBlacklistRequest{
		UserUuid:   targetUUID,
		TargetUuid: fromUUID,
	})
	if err != nil {
	...
	friendResp, err := c.friendClient.CheckIsFriend(ctx, &userpb.CheckIsFriendRequest{
		UserUuid: fromUUID,
		PeerUuid: targetUUID,
	})
	if err != nil {
