...
- `AddMember` / `LeaveGroup` / `RemoveMember` / `GetMemberList` / `SearchGroupMembers`。
- `TransferGroupOwner` / `UpdateMemberRole` / `MuteGroupMember` / `UpdateGroupMuteSetting`。
...
- msg 发送群消息前调用 group 权限能力校验成员身份、禁言和群状态。
