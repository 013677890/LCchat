...
**请求示例**:
```
GET /api/v1/auth/user/profile/user-uuid-002
```
...
**请求信息**:
```
POST /api/v1/auth/user/change-telephone
```
...
**接口描述**: 通过邮箱/昵称/用户ID搜索用户（仅返回公开信息，不返回 email）
...
**请求示例**:
```
GET /api/v1/auth/user/search?keyword=zhangsan&page=1&pageSize=20
```
...
**说明**:
- 搜索结果不返回 email / telephone
- `isFriend` 由网关聚合好友关系后填充
