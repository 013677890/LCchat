...
| 客户端 ↔ Connect | WebSocket Binary, `MessageEnvelope` Proto | 心跳、ACK 等连接控制帧，不再接收 JSON Text 帧 |
...
心跳使用 **WebSocket Binary 帧 + Protobuf**，外层为 `proto/connect/connect.proto` 中的 `MessageEnvelope`：

```protobuf
// 上行（客户端发）
MessageEnvelope{ type: "heartbeat" }

// 下行（服务端回）
MessageEnvelope{ type: "heartbeat_ack", server_ts: <unix_ms> }
```

> 当前协议不再接收 Text JSON 心跳；发送 `{ "type": "heartbeat" }` 会被视为格式错误。

### 2.4 下行帧格式

Connect 推送和连接控制消息统一使用 **WebSocket Binary 帧 + Protobuf** 编码（详见 [§4 下行推送](#4-下行推送消息到达客户端)）。
...
Push-Job 仅对 `MSG_PUSH` 且 `seq > 0` 的下行帧设置 `ack_required=true`。
...
客户端处理完需要 ACK 的下行消息后，应通过 **WebSocket Binary + Protobuf** 回送：

```protobuf
// 外层仍是 proto/connect/connect.proto
MessageEnvelope {
  type: "MSG_ACK"
  data: proto.Marshal(MessageAck{conv_id: <conv_id>, seq: <max_contiguous_seq>, msg_id: <optional>})
  seq:  <max_contiguous_seq>
}

// ACK payload 定义见 proto/connect/ws_control.proto
message MessageAck {
  string conv_id = 1;
  int64  seq     = 2;
  string msg_id  = 3;
...
服务端 ACK 校验规则：
- `conv_id` 不能为空，`seq` 必须大于 0；
- `seq` 必须小于等于当前 WebSocket 连接已成功下发过的该会话最大 `seq`；
- 校验失败返回 `MessageEnvelope{type:"error"}`，不会写 Redis ACK 位点；
- 校验通过后按 `user_uuid + device_id + conv_id` 写入 Redis ACK 位点，且只允许单调前进。
...
MessageEnvelope {
  type: "MSG_ACK_ACK"
  data: proto.Marshal(MessageAckAck{conv_id: <conv_id>, seq: <stored_seq>})
  seq:  <stored_seq>
}
```
