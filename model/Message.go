...
type Message struct {
	...
	ConvId       string         `gorm:"column:conv_id;type:varchar(128);not null;uniqueIndex:uidx_conv_seq;index:idx_conv_time;comment:会话ID,关联 conversation.conv_id"`
	Seq          int64          `gorm:"column:seq;not null;uniqueIndex:uidx_conv_seq;comment:会话内序号"`
	MsgId        string         `gorm:"column:msg_id;type:char(64);uniqueIndex:uk_message_msg_id;not null;comment:全局消息ID(雪花/UUID)"`
